...
class StatsDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for player_id, player_data in self.game_stats.items():
            ...
            emoji = get_team_emoji(
                interaction.guild,
                self.teams_json,
                team_name
            )

            verified_id = find_verified_user(
                stats_json,
                player_data
            )
...
class StatsView(discord.ui.View):
    def __init__(self, game_stats, teams_json):
        ...
        self.add_item(
            StatsDropdown(game_stats, teams_json)
        )
...
class ScoreReport(commands.Cog):
    ...
    async def scorereport(
        ...
        if away_team_id not in records_json:
            records_json[away_team_id] = {
                "wins": 0,
                "losses": 0,
                "points_for": 0,
                "points_against": 0
        ...
        if home_team_id not in records_json:
            records_json[home_team_id] = {
                "wins": 0,
                "losses": 0,
                "points_for": 0,
                "points_against": 0
        ...
        embed.add_field(
            name="Score",
            value=(
                f"{away_team.mention}: {away_score_text}\n"
                f"{home_team.mention}: {home_score_text}"
            ),
            inline=False
        )
        ...
        stats_channel = self.bot.get_channel(
            STATS_CHANNEL
        )
